...
Firmware upload and runtime testing require physical hardware, but **compilation/build is fully supported in the cloud environment** using PlatformIO.
...
- **Two separate `platformio.ini` files**: The root `platformio.ini` has `src_dir = teensy_firmware` so it builds the Teensy firmware. The ESP32-S3 env defined there will fail because it tries to compile Teensy sources. Always build the ESP32-S3 firmware from inside `esp32_firmware/` which has its own `platformio.ini`.
