...
- **Diagram**: ASCII representation of the 32 LED strip, showcasing how LED 0 is reserved as a sacrificial level-shift LED; LEDs 1-31 are used for display with a hardware level shifter. LED 0 (bottom) displays row 0, and LED 31 (top) displays row 31.
...
Images are converted to **32 pixels tall**, matching the 31 display LEDs (LED 0-LED 31). Each image column is streamed as a time slice while the poi spins. For every column:
