...
const NUM_LEDS = 31;   // Display LEDs (matching default g_displayLeds)
...
const PatternCanvas: React.FC<PatternCanvasProps> = ({
  patternId, speed, color1, color2, playing, audioLevel, height = 200,
}) => {
  ...
  const getState = useCallback((): PatternState => {
    if (!stateRef.current[patternId]) {
      stateRef.current[patternId] = {
        heat: new Array(NUM_LEDS).fill(0),
        cometPos: 0,
        cometDir: 1,
        ...
        lastAudioLevel: 0,
        rainbowOffset: 0,
        sparkleBuffer: new Array(NUM_LEDS).fill(null).map((): RGB => [0, 0, 0]),
      };
    }
  ...
  useEffect(() => {
    ...
    const LED_W = Math.floor(canvas.width / NUM_LEDS);
    const LED_GAP = 1;
    ...
    const render = () => {
      ...
      const st = getState();
      const leds: [number, number, number][] = new Array(NUM_LEDS).fill(null).map(() => [0, 0, 0]);

      switch (patternId) {
        // ── 0: Rainbow ─────────────────────────────────
        case 0:
          for (let i = 0; i < NUM_LEDS; i++) {
            const hue = (patternTime * speed / 10 + i * 255 / NUM_LEDS) % 256;
        ...
        case 1:
          for (let i = 0; i < NUM_LEDS; i++) {
            const brightness = sin8(patternTime * speed / 10 + i * 255 / NUM_LEDS);
        ...
        case 2: {
          ...
          for (let i = 0; i < NUM_LEDS; i++) {
            const phase = ((i * 255 / NUM_LEDS) + timeOffset) & 0xFF;
        ...
        case 3:
          ...
          for (let i = 0; i < NUM_LEDS; i++) leds[i] = copyRgb(st.sparkleBuffer[i]);
          break;
        ...
        case 4: {
          // Cool down
          for (let i = 0; i < NUM_LEDS; i++) {
            st.heat[i] = Math.max(0, st.heat[i] - Math.random() * (((55 * 10) / NUM_LEDS) + 2));
          }
          // Heat rises
          for (let i = NUM_LEDS - 1; i >= 2; i--) {
            st.heat[i] = (st.heat[i - 1] + st.heat[i - 2] + st.heat[i - 2]) / 3;
          }
          // Random ignition at bottom
          if (Math.random() * 255 < speed) {
            const y = Math.floor(Math.random() * Math.min(3, NUM_LEDS));
        ...
        case 5: {
          ...
          if (st.cometPos >= NUM_LEDS - 1 || st.cometPos <= 0) {
            st.cometDir = -st.cometDir;
          }
          st.cometPos = clamp(st.cometPos, 0, NUM_LEDS - 1);
        ...
        case 8: {
          ...
          st.meteorPos--;
          if (st.meteorPos < 0) st.meteorPos = NUM_LEDS - 1;
          for (let i = 0; i < NUM_LEDS; i++) leds[i] = copyRgb(st.sparkleBuffer[i]);
          break;
        }
        ...
        case 11: {
          ...
          const ledsToLight = Math.round(al / 255 * NUM_LEDS);
          for (let i = 0; i < NUM_LEDS; i++) {
            if (i < ledsToLight) {
              let hue: number;
              if (i < NUM_LEDS / 3) hue = 96;       // Green
              else if (i < 2 * NUM_LEDS / 3) hue = 64; // Yellow
              else hue = 0;                            // Red
          ...
          // Peak indicator
          const peakPos = Math.round(st.peakLevel / 255 * (NUM_LEDS - 1));
          if (peakPos >= 0 && peakPos < NUM_LEDS) {
        ...
        case 13: {
          ...
          for (let i = 0; i < NUM_LEDS; i++) {
            const hue = (Math.floor(st.rainbowOffset / 4) + i * 255 / NUM_LEDS) & 0xFF;
        ...
        case 14: {
          const al = audioLevel;
          const expansion = Math.round(al / 255 * (NUM_LEDS / 2));
          ...
          for (let i = 0; i <= expansion; i++) {
            ...
            if (center + i < NUM_LEDS) st.sparkleBuffer[center + i] = rgb;
        ...
        // ── 16: Split Spin ────────────────────────────
        case 16: {
          const offset = Math.floor(patternTime * speed / 20) % NUM_LEDS;
          const splitPoint = Math.floor(NUM_LEDS / 2);
          for (let i = 0; i < NUM_LEDS; i++) {
            const pos = (i + offset) % NUM_LEDS;
      ...
      const totalGap = (NUM_LEDS - 1) * LED_GAP;
      const ledWidth = (canvas.width - totalGap - 4) / NUM_LEDS;
  ...
  }, [patternId, speed, color1, color2, playing, audioLevel, getState, height]);
...
function useSimulatedAudio(enabled: boolean): number {
  ...
  useEffect(() => {
    ...
    const tick = () => {
      ...
      const bass = Math.max(0, Math.sin(t * 2 * Math.PI * 1.8) * 0.7);           // ~1.8 Hz bass beat
      const mid  = Math.max(0, Math.sin(t * 2 * Math.PI * 3.2 + 1.3) * 0.4);     // ~3.2 Hz mid
      const hi   = Math.max(0, Math.sin(t * 2 * Math.PI * 7.5 + 0.7) * 0.2);     // high hat
      const envelope = 0.5 + 0.5 * Math.sin(t * 2 * Math.PI * 0.15);             // slow envelope
      const noise = Math.random() * 0.08;                                          // noise floor
