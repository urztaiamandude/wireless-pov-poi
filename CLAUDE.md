...
These are non-negotiable. Every edit to firmware code MUST respect them.

1. **LED 0 is sacred.** It is a level-shift LED, NOT a display pixel. Every LED loop starts at index 1. `for (int i = 0; ...` in display code is always a bug.
2. **Display height is 31 pixels** (LEDs 1-31). Use `DISPLAY_LEDS` (31) and `DISPLAY_LED_START` (1). Never use `NUM_LEDS` (32) for display height math.
3. **Two firmware implementations exist.** `teensy_firmware/` (Arduino IDE, single file, complete) and `firmware/teensy41/` (PlatformIO, modular, partial port). Know which one you're editing. Don't mix patterns between them.
4. **Teensy and ESP32 are separate processors** connected by serial UART at 115200 baud. Never write code that assumes shared memory or direct function calls between them.
5. **Image pixels are `pixels[x][y]`** where x = column (horizontal, variable width) and y = LED index (vertical, 1-31). No flip needed. `leds[y] = pixels[column][y]`.
6. **Test at low brightness first** (< 50). Full-white all-LEDs draws 2-3A. Always respect current limits.
7. **Common ground is mandatory.** Teensy, ESP32, and LED strip share a common ground rail. Code that configures pins must never break this assumption.
...
pio run -e teensy41 -t upload  # Build + upload Teensy
```
